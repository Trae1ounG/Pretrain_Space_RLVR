...
class ActorWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: ActorConfig):
        import verl
        print(f"DEBUG: ActorWorker init, verl_path={verl.__file__}", flush=True)
        self.config = config
    ...
    def _build_engine(self):
        print(f"DEBUG: Building engine with strategy: {self.config.strategy}")
        ...
        with self.engine.train_mode():
            ...
            with Timer(name="update_policy", logger=None) as timer:
                for batch_idx, mini_batch in enumerate(dataloader):
                    print(f"DEBUG: update_actor batch {batch_idx}", flush=True)
                    mini_batch_metrics = self.engine.train_batch(mini_batch, self.loss_fn)
                    print(f"DEBUG: mini_batch_metrics keys: {list(mini_batch_metrics.keys())}", flush=True)
