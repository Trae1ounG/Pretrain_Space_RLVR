...
class FSDPEngineWithLMHead(FSDPEngine):
    def _forward_pass_core(self, micro_batch_tensor, temperature, calculate_entropy, use_fused_kernels, use_remove_padding):
        device_name = get_device_name()
    ...
    def forward_step(self, micro_batch: DataProto, loss_function, forward_only):
        print(f"DEBUG: forward_step meta_info keys: {list(micro_batch.meta_info.keys())}", flush=True)
        print(f"DEBUG: pretrain_loss_coef: {micro_batch.meta_info.get('pretrain_loss_coef')}", flush=True)
        print(f"DEBUG: has_pretrain_old_log_probs: {'pretrain_old_log_probs' in micro_batch.batch.keys()}", flush=True)
        ...
        log_probs, entropy = self._forward_pass_core(
            micro_batch_tensor, temperature, calculate_entropy, use_fused_kernels, use_remove_padding
        )
        ...
        if forward_only:
            ...
            pretrain_loss_coef = micro_batch.meta_info.get("pretrain_loss_coef", 0.0)
            pretrain_only = micro_batch.meta_info.get("pretrain_only", False)
            if pretrain_loss_coef > 0.0 and "pretrain_old_log_probs" in micro_batch.batch.keys():
                # Construct A-only batch tensor dict
                # Shallow copy dict
                pretrain_batch_tensor = {k: v for k, v in micro_batch_tensor.items()}

                responses = pretrain_batch_tensor["responses"]
                response_len = responses.shape[1]

                pretrain_batch_tensor["input_ids"] = responses
                pretrain_batch_tensor["attention_mask"] = micro_batch_tensor["attention_mask"][:, -response_len:]
                if "position_ids" in pretrain_batch_tensor:
                    pretrain_batch_tensor["position_ids"] = torch.arange(response_len, device=responses.device).unsqueeze(0).expand(responses.shape[0], -1)

                pretrain_log_probs, _ = self._forward_pass_core(
                    pretrain_batch_tensor, temperature, False, use_fused_kernels, use_remove_padding
                )

                pretrain_output = {"log_probs": pretrain_log_probs}

                pretrain_data = pretrain_batch_tensor.copy()
                pretrain_data["old_log_probs"] = micro_batch.batch["pretrain_old_log_probs"]
                if "pretrain_ref_log_probs" in micro_batch.batch.keys():
                    pretrain_data["ref_log_prob"] = micro_batch.batch["pretrain_ref_log_probs"]
                
                # Fix missing data for ppo_loss
                pretrain_data["response_mask"] = pretrain_batch_tensor["attention_mask"]
                if "advantages" in micro_batch.batch:
                    pretrain_data["advantages"] = micro_batch.batch["advantages"]

                print(f"Executing Pretrain Loss. Coef: {pretrain_loss_coef}, Only: {pretrain_only}", flush=True) 
                pretrain_loss, pretrain_metrics = loss_function(model_output=pretrain_output, data=pretrain_data)

                if pretrain_only:
                    policy_loss = pretrain_loss_coef * pretrain_loss
                else:
                    policy_loss = policy_loss + pretrain_loss_coef * pretrain_loss

                for k, v in pretrain_metrics.items():
                    metrics[f"pretrain/{k}"] = v
