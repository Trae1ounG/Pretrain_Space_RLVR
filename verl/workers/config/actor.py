...
class ActorConfig(BaseConfig):
    ...
    pretrain_loss_agg_mode: str = None  # If None, uses loss_agg_mode; otherwise uses this for pretrain loss
    entropy_coeff: float = 0
    entropy_mask: bool = False
    prefix_mask_ratio: float = 0.0
    negative_pretrain: bool = False
    positive_pretrain: bool = False
    negative_rl: bool = True
    positive_rl: bool = True
    mix_prob_alpha: float = 1.0
    ...
    def __post_init__(self):
        ...
        # Validate pretrain_loss_agg_mode if specified
        if self.pretrain_loss_agg_mode is not None and self.pretrain_loss_agg_mode not in valid_loss_agg_modes:
            raise ValueError(f"Invalid pretrain_loss_agg_mode: {self.pretrain_loss_agg_mode}")
...
class FSDPActorConfig(ActorConfig):
    ...
    Args:
        ...
        internal_layer (int): The layer number of internal layer policy
        internal_policy_interative (bool): Whether to use Bottom-up Policy Optimization
        iterative_steps (int): Steps of internal policy optimization
    """
    ...
    profiler: ProfilerConfig = field(default_factory=ProfilerConfig)
    internal_layer:int = 30
    internal_policy_interative: bool=False
    iterative_steps:int = 30
    entropy_mask: bool = False
    prefix_mask_ratio: float = 0.0
    negative_pretrain: bool = False
    positive_pretrain: bool = False
    negative_rl: bool = True
    positive_rl: bool = True
    mix_prob_alpha: float = 1.0  # Default to unconditional (current behavior)
    self_distill:float = False
