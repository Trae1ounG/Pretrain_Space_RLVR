...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            actor_module.config.model_type = "qwen2_custom"
            # breakpoint()
        ...
        print(f"FSDP Config: {self.config.actor.strategy}")
        ...
        if role == "actor" and optim_config is not None:
            ...
            training_parameters = actor_module_fsdp.parameters()
            ...
            elif warmup_style == "step":
                actor_lr_scheduler = get_step_schedule_with_warmup(
                    optimizer=actor_optimizer,
                    num_warmup_steps=num_warmup_steps,
                    min_lr_ratio=min_lr_ratio,
                )
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def update_ref_from_actor(self):
        """
        Update the reference model with the current actor model weights.
        This allows resetting the reference model after pretraining or at specific intervals.
        """
        if not self._is_ref:
            return
        ...
        # Ensure both models are available and have compatible structures
        # In FSDP, we assume actor and ref have the same sharding strategy and structure
        if not hasattr(self, "actor_module_fsdp") or not hasattr(self, "ref_module_fsdp"):
            return

        log_with_rank(
            f"[rank-{self.rank}]: Syncing Ref Model with Actor Model weights...",
            rank=self.rank,
            logger=logger
        )

        with torch.no_grad():
            # Iterate through named parameters to ensure matching
            # Note: We assume the order and names are identical.
            # Using zip strictly relies on the order.
            for (name_ref, param_ref), (name_actor, param_actor) in zip(
                self.ref_module_fsdp.named_parameters(), 
                self.actor_module_fsdp.named_parameters()
            ):
                # Basic safety check
                if param_ref.shape != param_actor.shape:
                    raise ValueError(f"Shape mismatch for {name_ref}: {param_ref.shape} vs {param_actor.shape}")
                
                # In-place copy
                param_ref.data.copy_(param_actor.data)
        
        # Barrier to ensure all ranks finish copying before proceeding
        if dist.is_initialized():
            dist.barrier()
            
        log_with_rank(
            f"[rank-{self.rank}]: Ref Model sync completed.",
            rank=self.rank,
            logger=logger
        )
