...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str):
    ...
    if loss_agg_mode == "token-mean":
        ...
        token_counts = torch.sum(loss_mask, dim=-1)  # (bsz,)
        seq_losses = torch.sum(loss_mat * loss_mask, dim=-1)  # token-sum
        # Exclude fully-masked sequences to avoid diluting the mean with zero-loss samples
        valid_seq_mask = token_counts > 0
        loss = seq_losses[valid_seq_mask].mean() if valid_seq_mask.any() else seq_losses.sum() * 0.0  # seq-mean
    elif loss_agg_mode == "seq-mean-token-mean":
        token_counts = torch.sum(loss_mask, dim=-1)  # (bsz,)
        seq_losses = torch.sum(loss_mat * loss_mask, dim=-1) / token_counts.clamp(min=1)  # token-mean, avoid div-by-zero
        # Exclude fully-masked sequences (token_counts == 0) from the seq-mean to avoid biasing loss toward 0
        valid_seq_mask = token_counts > 0
        loss = seq_losses[valid_seq_mask].mean() if valid_seq_mask.any() else seq_losses.sum() * 0.0  # seq-mean
...
def compute_self_distillation_loss(
    ...
    return loss, torch.tensor(0.0), torch.tensor(0.0), torch.tensor(0.0)
...
@register_policy_loss("vanilla_self_distill_w_clip")  # type: ignore[arg-type]
def compute_policy_loss_vanilla_self_distill_w_clip(
        student_log_probs=None, #可以决定到底detach P(y)还是P(y|x)
    ...
    ratio_mask = torch.logical_and(
        torch.exp(teacher_log_probs - student_log_probs) < 1.2,
        torch.exp(teacher_log_probs - student_log_probs) > 0.8,
    )
    
    student_log_probs = student_log_probs.unsqueeze(-1)
    ...
    per_token_loss = kl_loss.sum(-1) * (-1) # 负的kl->max kl
    
    response_mask = response_mask & ratio_mask
    pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
    
    # Return dummy values for clipping metrics since we're not using them
...
@register_policy_loss("vanilla_wo_is")  # type: ignore[arg-type]
def compute_policy_loss_vanilla_wo_is(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    """
    Compute vanilla policy gradient loss without importance sampling.
    
    This is a simplified version that only uses current policy's log probabilities
    without the importance sampling ratio (no clipping, no ratio computation).
    
    Args:
        old_log_prob (torch.Tensor): Log-probabilities of actions under the old policy, 
            shape (batch_size, response_length). (Not used in this version)
        log_prob (torch.Tensor): Log-probabilities of actions under the current policy, 
            shape (batch_size, response_length).
        advantages (torch.Tensor): Advantage estimates for each action, 
            shape (batch_size, response_length).
        response_mask (torch.Tensor): Mask indicating which tokens to include in the loss, 
            shape (batch_size, response_length).
        loss_agg_mode (str, optional): Aggregation mode for `agg_loss`. Defaults to "token-mean".
        config: `(verl.trainer.config.ActorConfig)`: config for the actor. (Not strictly needed)
        rollout_log_probs: `(torch.Tensor)`: log probabilities of actions under the rollout policy. 
            (Not used in this version)
    
    Returns:
        tuple: (pg_loss, metrics_dict)
            - pg_loss: The policy gradient loss
            - metrics_dict: Dictionary containing dummy metrics
    """
    # Simple policy gradient loss: -log_prob * advantages
    advantages = torch.clamp(
        advantages, 1 - 0.2, 1 + 0.2
    )
    ...
    return pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower
@register_policy_loss("vanilla_self_distill")  # type: ignore[arg-type]
def compute_policy_loss_vanilla_wo_is(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    ...
    #     advantages, 1 - 0.2, 1 + 0.2
    # )
    # pg_losses = -1 * torch.exp(log_prob) * advantages
    # mix log prob版本, adv是mix的prob
    pg_losses = (torch.exp(log_prob) * (log_prob - advantages) * 0.5 + torch.exp(old_log_prob) * (old_log_prob - advantages) * 0.5)
...
@register_policy_loss("vanilla_self_distill_new")  # type: ignore[arg-type]
def compute_policy_loss_vanilla_wo_is(
    student_log_probs: torch.Tensor,
    teacher_log_probs: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    if alpha == 0.0:
        ...
        mixture_log_probs = torch.logsumexp(
            torch.stack([student_log_probs + torch.log(1 - alpha), teacher_log_probs + torch.log(alpha)]),
            dim=0,
        )
    ...
    per_token_loss = kl_loss.sum(-1)
    # first_500_mask = torch.ones_like(response_mask, dtype=torch.bool)
    # first_500_mask[:, :500] = False
    # response_mask = response_mask & first_500_mask
...
@register_policy_loss("higher")
def compute_policy_loss_higher(
    old_log_prob,
    log_prob,
    advantages,
    response_mask,
    ...
    pg_losses = torch.where(advantages < 0, clip_pg_losses2, clip_pg_losses1)
    pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=entropy_mask, loss_agg_mode=loss_agg_mode)
