...
def unbiased_pass_at_k(results: List[bool], k: int) -> float:
    """
    Calculate unbiased pass@k estimate using the standard formula.
    
    The unbiased estimator is: pass@k = E[1 - comb(n-c, k) / comb(n, k)]
    where n is total samples and c is number of correct samples.
    ...
    # Unbiased estimator: 1 - P(all k samples are failures)
    # P(all k failures) = comb(n-c, k) / comb(n, k)
    if n - c < k:  # Not enough failures to select k failure samples
        return 1.0
    
    # try:
    prob_all_fail = comb(n - c, k) / comb(n, k)
    return 1.0 - prob_all_fail
    # except (OverflowError, ZeroDivisionError):
    #     # Fallback for numerical issues
    #     return c / n  # Simple success rate
...
def process_validation_metrics(
    ...
    seed: int = 42,
    pass_threshold: float = 0.5,  # 新增参数：通过测试的阈值
    pass_field: str = "reward"     # 新增参数：用于判断通过的字段名
    ...
    majority voting results, and pass@k metrics.
    ...
    Args:
        ...
        pass_threshold: Threshold for determining if a sample passes (for pass@k calculation).
        pass_field: Field name to use for pass@k calculation.
    ...
    Returns:
        ...
        Where metric_name includes all original metrics plus:
        - "pass@4": Pass@4 metric
        - "pass@8": Pass@8 metric
        - "pass@4/std": Bootstrap standard deviation for pass@4
        - "pass@8/std": Bootstrap standard deviation for pass@8
    """
    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                ...
                # 添加 pass@k 计算
                if var_name == pass_field or (pass_field in var2vals and var_name == pass_field):
                    # 将数值转换为布尔值（是否通过测试）
                    pass_results = [val >= pass_threshold for val in var_vals]
                    
                    # Calculate Pass@K for K = 1, 2, 4, ... 2^i <= n_resps
                    current_k = 1
                    while current_k <= n_resps:
                        pass_k = unbiased_pass_at_k(pass_results, current_k)
                        metric[f"pass@{current_k}"] = pass_k
                        current_k *= 2
                    # Debug print
                    print(f"Computed Pass@K for {data_source}/{uid}: n={n_resps}, metrics={metric.keys()}")
